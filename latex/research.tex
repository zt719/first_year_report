...
\input{latex/background1}
\input{latex/background2}
...
%
\\[\AgdaEmptyExtraSkip]%
\>[0]\AgdaComment{--\ commute\ :\ (β\ :\ ⊤\ ⊎\ X\ →\ X)\ (x\ :\ ⊤\ ⊎\ ℕ)}\<%
\\
\>[0]\AgdaComment{--\ \ \ →\ fold\ β\ ([z,s]\ x)\ ≡\ β\ (⊤⊎₁\ (fold\ β)\ x)}\<%
\\
\>[0]\AgdaComment{--\ commute\ β\ (inj₁\ tt)\ =\ refl}\<%
\\
\>[0]\AgdaComment{--\ commute\ β\ (inj₂\ n)\ =\ refl}\<%
\end{code}
